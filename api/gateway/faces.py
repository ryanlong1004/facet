...
class FaceDataHandler:
    ...
    Attributes:
        FACES_PICKLE_FILE_PATH (str): The path to the pickle file.
    """

    def __init__(self, faces_pick_file_path: str):
        """
        ...
        Args:
            FACES_PICKLE_FILE_PATH (str): The path to the pickle file.
        """
        self.FACES_PICKLE_FILE_PATH = faces_pick_file_path
    ...
    def _initialize_pickle_file(self):
        ...
        """
        if not os.path.exists(self.FACES_PICKLE_FILE_PATH):
            logger.info("Initializing pickle file at %s", self.FACES_PICKLE_FILE_PATH)
            self._write_pickle({})
    ...
    def _read_pickle(self) -> Dict[str, FaceData]:
        ...
        """
        try:
            with open(self.FACES_PICKLE_FILE_PATH, "rb") as f:
                logger.debug(
                    "Reading data from pickle file: %s", self.FACES_PICKLE_FILE_PATH
                )
                return pickle.load(f)
        except FileNotFoundError:
            logger.error("Pickle file not found: %s", self.FACES_PICKLE_FILE_PATH)
            raise
        except Exception as e:
    ...
    def _write_pickle(self, data: Dict[str, FaceData]):
        ...
        """
        try:
            with open(self.FACES_PICKLE_FILE_PATH, "wb") as f:
                pickle.dump(data, f)
                logger.debug(
                    "Data written to pickle file: %s", self.FACES_PICKLE_FILE_PATH
                )
        except Exception as e:
